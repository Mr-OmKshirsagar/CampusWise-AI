...
export default function SourceDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      {/* Frosted Backdrop */}
      <div
        className="absolute inset-0 bg-black/50 dark:bg-[#030508]/80 backdrop-blur-md transition-opacity animate-fade-in"
        onClick={closeSourceDrawer}
      />

      <div className="fixed inset-y-0 right-0 max-w-full flex">
        <div className="w-screen sm:max-w-md md:max-w-lg glass-panel-elevated border-l border-slate-200 dark:border-white/[0.12] shadow-2xl flex flex-col animate-slide-in-right bg-white/95 dark:bg-[#070b12]/95 backdrop-blur-2xl transition-colors duration-300 h-full">
          {/* Drawer Header */}
          <div className="p-3.5 sm:p-5 border-b border-slate-200 dark:border-white/[0.08] flex items-center justify-between bg-slate-50/50 dark:bg-white/[0.02] shrink-0">
            ...
            <button
              onClick={closeSourceDrawer}
              className="p-2 rounded-xl glass-badge text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all active:scale-95 shrink-0"
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5 no-scrollbar">
            ...
            <div className="glass-card p-4 sm:p-5 rounded-2xl space-y-3.5 sm:space-y-4 border-slate-200 dark:border-white/[0.1]">
              <div className="flex items-start justify-between gap-2.5">
                ...
                <span className="px-2.5 py-1 bg-sky-500/15 dark:bg-sky-500/20 border border-sky-500/35 rounded-xl text-[11px] sm:text-xs font-mono font-bold text-sky-700 dark:text-sky-300 shadow-sm shrink-0">
              ...
              {selectedSource.similarity_score && (
                <div className="pt-3 border-t border-slate-200 dark:border-white/[0.08] space-y-2">
                  ...
                  <div className="w-full bg-slate-200 dark:bg-slate-900 rounded-full h-2 sm:h-2.5 overflow-hidden border border-slate-300/60 dark:border-white/[0.06] p-0.5">
                    <div
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 hover:text-sky-600 dark:hover:text-sky-300 transition-colors px-2 py-1 rounded-lg hover:bg-black/5 dark:hover:bg-white/[0.08]"
                >
                  {copied ? (
                    <>
              ...
              <div className="p-3.5 sm:p-5 glass-input rounded-2xl text-xs sm:text-sm text-slate-800 dark:text-slate-200 leading-relaxed font-sans whitespace-pre-wrap selection:bg-sky-500 selection:text-white border-slate-200 dark:border-white/[0.1] break-words">
                {selectedSource.excerpt || 'Full excerpt chunk is stored in the PostgreSQL pgvector database.'}
              </div>
            </div>

            {/* Anti-Hallucination Verified Badge */}
            <div className="p-3.5 sm:p-4 rounded-2xl bg-sky-500/10 dark:bg-sky-950/30 border border-sky-500/25 text-xs text-sky-900 dark:text-sky-200/90 leading-relaxed flex items-start gap-2.5 sm:gap-3 shadow-sm">
          ...
          <div className="p-3.5 sm:p-4 border-t border-slate-200 dark:border-white/[0.08] bg-slate-50/50 dark:bg-white/[0.02] shrink-0">
            <button
              onClick={closeSourceDrawer}
              className="w-full py-2.5 sm:py-3 rounded-xl glass-badge hover:bg-black/5 dark:hover:bg-white/[0.1] text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-all active:scale-98"
            >
