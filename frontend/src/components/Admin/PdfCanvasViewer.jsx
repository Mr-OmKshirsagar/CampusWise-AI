...
export default function PdfCanvasViewer({
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-950 text-white relative overflow-hidden select-none">
      {/* 1. TOP LIQUID GLASS FLOATING TOOLBAR */}
      <div className="flex items-center justify-between px-3 sm:px-5 py-2.5 bg-white/90 dark:bg-[#070b12]/95 backdrop-blur-2xl border-b border-slate-200/90 dark:border-white/[0.1] shadow-liquid-sm shrink-0 z-20 transition-colors">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          {numPages > 0 && (
            <span className="hidden xs:inline-block text-[10px] font-mono font-bold px-2 py-0.5 rounded-full glass-badge text-sky-700 dark:text-sky-300">
        ...
        {numPages > 1 && (
          <div className="flex items-center gap-1 glass-badge rounded-full px-2 py-1 shadow-sm">
            <button
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          {/* Zoom Engine */}
          <div className="hidden sm:flex items-center gap-1 glass-badge rounded-full px-1.5 py-0.5">
            <button
      ...
      {/* 2. SCROLLABLE CANVAS DOCUMENT BODY */}
      <div
        ref={containerRef}
        onScroll={handleScroll}
        className="flex-1 w-full h-full min-h-0 overflow-y-auto overscroll-contain touch-scroll-momentum p-3 sm:p-6 space-y-4 sm:space-y-6 flex flex-col items-center bg-[#070b12]"
        style={{
      ...
      >
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full my-auto p-6 text-center space-y-4 max-w-md">
